...
} = await import('../../../../express/blocks/marquee-exp-1/marquee-exp-1.js');
setConfig({});
...
async function prepBlock(filePath) {
  ...
  const marquee = document.querySelector('.marquee-exp-1');
...
describe('marquee', () => {
  describe('default version', () => {
    it('has a video background', async () => {
      ...
      const video = marquee.querySelector('video.marquee-exp-1-background');
      expect(video).to.exist;
    });

    it('has a content foreground', async () => {
      const marquee = await prepBlock('./mocks/default.html');
      const content = marquee.querySelector('.marquee-exp-1-foreground .content-wrapper');
      expect(content).to.exist;
    });

    it('has at least an H1', async () => {
      const marquee = await prepBlock('./mocks/default.html');
      const h1 = marquee.querySelector('.marquee-exp-1-foreground .content-wrapper h1');
      expect(h1).to.exist;
    });
    ...
    it('window resize triggers video adjustment', async () => {
      ...
      const newVideo = marquee.querySelector('video.marquee-exp-1-background');
