import { getConfig, loadScript, fetchPlaceholders } from '../../scripts/utils.js';
...
export async function buildCta(el, params) {
  ...
  const phText = el.textContent?.replace(/^CTA +/, '').replace('{{', '').replace('}}', '');
  const placeholders = await fetchPlaceholders();
  const text = placeholders[phText];
