...
function createAnimation(animations) {
  const attribs = {
    class: 'marquee-exp-1-background',
    playsinline: '',
    autoplay: '',
    muted: '',
  };
  ...
  for (const div of rows) {
    ...
    if (rowType === 'content') {
      ...
      if (video) {
        ...
        bg = createTag('div');
        bg.classList.add('marquee-exp-1-background');
        div.prepend(bg);
      }

      const marqueeForeground = createTag('div', { class: 'marquee-exp-1-foreground' });
      ...
      if (legalCopy && legalCopy.previousElementSibling?.classList.contains('buttons-wrapper')) {
        ...
        const legalText = createTag('span', { class: 'entitled-text' }, legalCopy.textContent.replace('**', '').trim());
        legalCopy.innerHTML = '';
