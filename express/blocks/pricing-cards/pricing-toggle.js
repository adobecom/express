...
export default function createToggle(placeholders, pricingSections) {
  ...
  toggleWrapper.setAttribute('aria-labelledby', 'radio-group-label');
  const groupLabel = toggleWrapper.children[0];
  groupLabel.setAttribute('id', 'radio-group-label');
  const buttons = PLANS.map((plan, i) => {
    ...
    button.setAttribute('aria-labeledby', plan);
    const label = plan[0].toUpperCase() + plan.slice(1).toLowerCase();
    button.append(createTag('div', { id: plan }, label));
